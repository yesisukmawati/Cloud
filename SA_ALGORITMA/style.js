const foods = [
    ...
    { name: "ayam geprek", price: 20000, protein: 25.00},
...
function findBestMealBranchAndBound(foods, budget, maxProtein, maxItems) {
    ...
    function bound(index, currentProtein, currentCost, remainingBudget) {
        ...
        return upperBound;
    } //menghitung batas atas (upper bound) jumlah protein yang dapat dicapai dari indeks tertentu
    ...
    function branchAndBound(index, currentMeal, currentProtein, currentCost) {
        if (index === foods.length || currentMeal.length === maxItems) {
            ...
            return;
        } //Jika batas atas ini tidak lebih besar dari jumlah protein dalam bestMeal saat ini, maka pencarian pada cabang tersebut dapat dipotong (pruned) karena tidak akan menemukan solusi yang lebih baik.
...
let start_backtracking = new Date()
findBestMealBacktracking(affordableFoods, budget, maxProtein, maxItems);
let end_backtracking = new Date()
let time_backtracking = end_backtracking - start_backtracking;
console.log(time_backtracking.toFixed(6))

let start_bnb = new Date()
findBestMealBranchAndBound(affordableFoods, budget, maxProtein, maxItems);
let end_bnb = new Date()
let time_bnb = end_bnb - start_bnb 
console.log(time_bnb.toFixed(6))
